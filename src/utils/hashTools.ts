...
type DigestAlgorithm = 'SHA-1' | 'SHA-256' | 'SHA-512'

type DigestFunction = (algorithm: DigestAlgorithm, text: string) => Promise<string>
type Md5Function = (text: string) => string
...
function convertToWordArray(input: string): number[] {
  ...
  for (let index = 0; index < bytes.length; index += 1) {
    ...
    words[wordIndex] = currentWord | (byte << ((index % 4) * 8))
  }

  const paddingIndex = bytes.length >>> 2
  words[paddingIndex] = (words[paddingIndex] ?? 0) | (0x80 << ((bytes.length % 4) * 8))
...
async function defaultDigest(algorithm: DigestAlgorithm, text: string): Promise<string> {
  const bytes = new TextEncoder().encode(text)
  const buffer = await crypto.subtle.digest(algorithm, bytes)
  return bytesToHex(new Uint8Array(buffer))
}

export async function generateHashes(text: string, overrides: HashOverrides = {}): Promise<HashResult> {
