...
function parseRgb(value: string): RgbTuple | null {
  ...
  if (!match) return null
  const rgb = match.slice(1).map(Number) as RgbTuple
  if (rgb.some((channel) => channel < 0 || channel > 255)) return null
  return rgb
}

function parseHsl(value: string): HslTuple | null {
...
function hslToRgb(hue: number, saturation: number, lightness: number): RgbTuple {
  ...
  return rgbPrime.map((channel) => Math.round((channel + m) * 255)) as RgbTuple
}
...
function rgbToCmyk(red: number, green: number, blue: number): CmykTuple {
  const r = red / 255
  const g = green / 255
  const b = blue / 255
  ...
  return [cyan, magenta, yellow, black * 100].map((value) => Math.round(clamp(value, 0, 100))) as CmykTuple
}

function parseToRgb(input: string): RgbTuple | null {
